			...
			if($_SERVER['REQUEST_METHOD'] == "POST") {
				...
				if($name != null && $email != null && $username != null && $password != null) {
					...
					if(mysqli_query($con,$query_addManufacturer)) {
						echo "<script> alert(\"Producents Added Successfully\"); </script>";
						header('Refresh:0');
					}
					else {
						$requireErr = "Adding Producents Failed";
					}
				}
				else {
...
<body>
	...
	<section>
		...
		<li>
			<input type="submit" value="Add Producents" class="submit_button" /> <span class="error_message"> <?php echo $requireErr; ?> </span><span class="confirm_message"> <?php echo $confirmMessage; ?> </span>
		</li>
		</ul>
		</form>
	</section>
