			...
			if($_SERVER['REQUEST_METHOD'] == "POST") {
				...
				if($name != null && $email != null && $username != null) {
					...
					if(mysqli_query($con,$query_UpdateMan)) {
						echo "<script> alert(\"Producents Details Updated Successfully\"); </script>";
						header('Refresh:0;url=view_manufacturer.php');
					}
					else {
						$requireErr = "Updating Producents Failed";
					}
				}
				else {
...
<body>
	...
	<section>
		...
		<li>
			<input type="submit" value="Update Producents" class="submit_button" /> <span class="error_message"> <?php echo $requireErr; ?> </span><span class="confirm_message"> <?php echo $confirmMessage; ?> </span>
		</li>
		</ul>
		</form>
	</section>
