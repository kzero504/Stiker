		...
		if($_SESSION['admin_login'] == true) {
			...
			if($_SERVER['REQUEST_METHOD'] == "POST") {
				if(isset($_POST['chkId'])) {
					...
					if(!$result) {
						echo "<script> alert(\"There was some problems deleting producents\"); </script>";
						header('Refresh:0');
					}
					else {
						echo "<script> alert(\"producents Deleted Successfully\"); </script>";
						header('Refresh:0');
					}
				}
			}
